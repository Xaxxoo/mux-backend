...
export function validateEnv(env: NodeJS.ProcessEnv): ValidatedEnv {
  ...
  const METRICS_SCRAPE_TOKEN = env.METRICS_SCRAPE_TOKEN?.trim() ?? '';
  const HORIZON_IMPORT_SECRET = env.HORIZON_IMPORT_SECRET?.trim() ?? '';
